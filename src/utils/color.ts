...
const INT_HEX_MAP: { [key: string]: string } = {
  10: "A",
  11: "B",
  12: "C",
  13: "D",
...
const HEX_INT_MAP: { [key: string]: number } = {
  A: 10,
  B: 11,
  C: 12,
  D: 13,
...
const parseHexChannel = function (hex: string) {
  if (hex.length === 2) {
    return (
      (HEX_INT_MAP[hex[0].toUpperCase() as string] || +hex[0]) * 16 +
...
export class Color {
  ...
  set(prop: { [key: string]: number } | string, value?: number): void {
